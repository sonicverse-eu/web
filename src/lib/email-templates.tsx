...
export type TeamContactEmailProps = {
  threadId: string;
  name: string;
  email: string;
  company?: string;
...
export type SubmitterContactEmailProps = {
  threadId: string;
  name: string;
  detailsUrl?: string;
  category: string;
...
const headerCellStyle: CSSProperties = { padding: '22px 24px', color: '#ffffff' };
const bodyCellStyle: CSSProperties = { padding: '20px 24px' };
...
const titleStyle: CSSProperties = { margin: '8px 0 0', fontSize: '24px', lineHeight: 1.2 };
const subtitleStyle: CSSProperties = { margin: '10px 0 0', fontSize: '14px', opacity: 0.92 };
...
const sectionTitleStyle: CSSProperties = { margin: '20px 0 8px', fontSize: '16px', color: '#2c1a52' };
...
const bodyCopyStyle: CSSProperties = { margin: '0 0 12px', fontSize: '15px', lineHeight: 1.65 };
const metaCopyStyle: CSSProperties = { margin: '0 0 12px', fontSize: '14px', color: '#3d2b63' };
...
const linkStyle: CSSProperties = { color: '#2358bb' };
...
function EmailShell({
  ...
  return (
    <div style={{ ...shellStyle, background }}>
      <table role="presentation" width="100%" style={cs}>
        <tbody>
          <tr>
...
export function TeamContactEmail({
  ...
  return (
    ...
    >
      ...
      <div style={{ ...messageBoxStyle, border: '1px solid #e7dfff', background: '#faf8ff' }}>
        {message}
      </div>
    </EmailShell>
  );
  ...
  return (
    ...
    >
      <p style={bodyCopyStyle}>
        Hi {name}, thanks for reaching out. Your request has been routed and tracked with this
        thread ID:
      </p>
      ...
      {detailsUrl ? (
        <p style={metaCopyStyle}>
          Reference URL:{' '}
          <a href={detailsUrl} style={linkStyle}>
            {detailsUrl}
          </a>
        </p>
      ) : null}
      <p style={messageLabelStyle}>Your message</p>
      <div style={{ ...messageBoxStyle, border: '1px solid #dbe8ff', background: '#f9fbff' }}>
        {message}
      </div>
